...
class Dao {
  ...
  insertMember = (q) => {
    let info = [q.username, q.password, q.servings];
    return this.sqlHandler(sqls.sql_insertMember, info);
  };
  getMemberByUsername = (username) =>
    this.sqlHandler(sqls.sql_getMemberByUsername, username, 1);

  getMemberByUserId = (id) =>
    this.sqlHandler(sqls.sql_getMemberByUserId, id, 1);
  ...
  getIngIdByRecipeId = (id) =>
    this.sqlHandler(sqls.sql_getIngIdByRecipeId, id, 1);

  getRecipeByIds = (id, memberId) => {
    let info = [id, memberId];
    return this.sqlHandler(sqls.sql_getRecipeByIds, info, 1);
  };
  getIngredientsById = (id) =>
    this.sqlHandler(sqls.sql_getIngredientsById, id, 1);

  getFromTable = (unitTableName) =>
    this.sqlHandler(sqls.sql_getFromTable, unitTableName, 1);

  getColumnNames = (unitTableName) =>
    this.sqlHandler(sqls.sql_getColumnNames, unitTableName);

  getRecipeByName = (name, memberId) => {
    let info = [name, memberId];
    return this.sqlHandler(sqls.sql_getRecipeByName, info, 1);
  };

  insertMaterialUnitColumn = (tableName, columnName) => {
    let info = [tableName, columnName];
