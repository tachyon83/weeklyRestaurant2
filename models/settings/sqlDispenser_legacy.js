...
let sql_createUser = `create user if not exists ${dbSetting.user}@'%' 
    identified by '${dbSetting.password}';`;
let sql_grantPrivileges = `grant all privileges on ${dbSetting.database}.* 
    to '${dbSetting.user}'@'%' with grant option;`;
let sql_flush = `flush privileges;`;
let sql_alterUser = `alter user ${dbSetting.user} 
...
let sql_createTable_fish_unit = `create table if not exists 
...
let sql_createTable_misc_unit = `create table if not exists 
...
let sql_createTable_sauce_unit = `create table if not exists 
...
let sql_insert_meat_unit = `insert into ${dbSetting.table_meat_unit} 
    values ('마리','kg','kg','컵');`;

let sql_insert_fish_unit = `insert into ${dbSetting.table_fish_unit} 
    values ('마리','마리','g','마리');`;

let sql_insert_misc_unit = `insert into ${dbSetting.table_misc_unit} 
    values('개','개','개','개','단','개','인분','인분','개','개','개',
    'g','g','g','g','g','개','개','개','뿌리','개','개','g','ml');`;

let sql_insert_sauce_unit = `insert into ${dbSetting.table_sauce_unit} 
    values('스푼','스푼','스푼','스푼','스푼','스푼','스푼','스푼',
    '스푼','스푼','스푼','컵');`;

// dummy data insertion

let sql_insert_meat1 = `insert into ${dbSetting.table_meat}(닭고기) 
    values (1);`;

let sql_insert_meat2 = `insert into ${dbSetting.table_meat}(닭고기) 
    values(1.5);`;

let sql_insert_meat3 = `insert into ${dbSetting.table_meat}(돼지고기) 
    values(1);`;

let sql_insert_meat4 = `insert into ${dbSetting.table_meat}(소고기) 
    values(1);`;

let sql_insert_meat5 = `insert into ${dbSetting.table_meat}(순대) 
    values(4);`;

let sql_insert_meat6 = `insert into ${dbSetting.table_meat}(돼지고기) 
    values(0.2);`;

let sql_insert_meat7 = `insert into ${dbSetting.table_meat}(돼지고기) 
    values(0.6);`;

let sql_insert_meat8 = `insert into ${dbSetting.table_meat}(소고기,돼지고기) 
    values(0.3,0.3);`;

let sql_insert_fish1 = `insert into ${dbSetting.table_fish}(멸치) 
    values(30);`;

let sql_insert_fish2 = `insert into ${dbSetting.table_fish}(새우,조개,오징어) 
    values(10,80,20);`;

let sql_insert_fish3 = `insert into ${dbSetting.table_fish}(새우,조개,오징어) 
    values(3,200,0.5);`;

let sql_insert_misc1 = `insert into ${dbSetting.table_misc}(감자,양파,당근,버섯,대파,마늘,면,국수,청양고추) 
    values (2, 0.5, 0.3, 0.5, 1, 0, 0, 0, 2);`;

let sql_insert_misc2 = `insert into ${dbSetting.table_misc}(양파,국수,애호박,참깨,김,우유) 
    values(0.5,1,0.5,1,1,0.5);`;

let sql_insert_misc3 = `insert into ${dbSetting.table_misc}(양파,대파,쌀,두부) 
    values(1,1,300,1);`;

let sql_insert_misc4 = `insert into ${dbSetting.table_misc}(양파,마늘,면,청양고추,애호박) 
    values(0.6,5,2,1,1);`;

let sql_insert_misc5 = `insert into ${dbSetting.table_misc}(당근,고추,참깨,쌀) 
    values(0.3,1,1,350);`;

let sql_insert_misc6 = `insert into ${dbSetting.table_misc}(양파,국수,애호박,참깨,김,우유) 
    values(0.7,1,0.5,1,1,0.5);`;

let sql_insert_misc7 = `insert into ${dbSetting.table_misc}(대파,청양고추) 
    values(1,2);`;

let sql_insert_misc8 = `insert into ${dbSetting.table_misc}(양파,대파,면,양배추,오이) 
    values(1, 1.5, 2, 0.2, 0.3);`;

let sql_insert_misc9 = `insert into ${dbSetting.table_misc}(양파,버섯,대파,참깨,쌀,죽순,파프리카,청경채) 
    values(0.5,2,1,10,200,1,1,4);`;

let sql_insert_misc10 = `insert into ${dbSetting.table_misc}(양파, 마늘, 달걀, 빵가루, 우유) 
    values(1, 1, 3, 200, 950);`;

let sql_insert_misc11 = `insert into ${dbSetting.table_misc}(양파,마늘,면) 
    values(1, 5, 1);`;

let sql_insert_misc12 = `insert into ${dbSetting.table_misc}(양파,버섯,달걀,빵가루) 
    values(1,6,2,200);`;

let sql_insert_sauce1 = `insert into ${dbSetting.table_sauce}(간장,고추장,고춧가루,설탕,소금,후추,식초,다진마늘) 
    values (1.5, 0, 1, 3, 0, 0, 0, 2);`;

let sql_insert_sauce2 = `insert into ${dbSetting.table_sauce}(간장,고추장,고춧가루,설탕,소금,후추,식초,다진마늘) 
    values (1.5, 0, 1, 3, 0, 1, 0, 2);`;

let sql_insert_sauce3 = `insert into ${dbSetting.table_sauce}(간장,소금) 
    values (10, 1.5);`;

let sql_insert_sauce4 = `insert into ${dbSetting.table_sauce}(간장,고추장,고춧가루,다진마늘,참기름) 
    values (0.3, 0.5, 3, 2, 1);`;

let sql_insert_sauce5 = `insert into ${dbSetting.table_sauce}(간장,고추장,설탕,소금,후추,식초,다진마늘,청주) 
    values (1, 1.5, 1, 0.3, 1, 1, 0.5, 2);`;

let sql_insert_sauce6 = `insert into ${dbSetting.table_sauce}(간장,고춧가루,다진마늘,청주,참기름) 
    values (3, 0.5, 0.5, 1, 1);`;

let sql_insert_sauce7 = `insert into ${dbSetting.table_sauce}(고춧가루,소금,후추,다진마늘,새우젓) 
    values (3, 0.5, 0.5, 1.5, 1.5);`;

let sql_insert_sauce8 = `insert into ${dbSetting.table_sauce}(설탕,춘장) 
    values (0.2, 0.3);`;

let sql_insert_sauce9 = `insert into ${dbSetting.table_sauce}(다진마늘,청주,참기름) 
    values (2, 2, 2);`;

let sql_insert_sauce10 = `insert into ${dbSetting.table_sauce}(소금,후추) 
    values (0.5, 0.3);`;

let sql_insert_sauce11 = `insert into ${dbSetting.table_sauce}(소금) 
    values (0.2);`;

let sql_insert_sauce12 = `insert into ${dbSetting.table_sauce}(간장,설탕,소금,후추,다진마늘) 
...
let sql_insertMaterialColumn = `alter table ?? add column ?? decimal(5,2);`;

let sql_insertMaterialUnit = `update ?? set ??=?;`;

let sql_findIdByMaterials = `select id from ?? 
    where ?;`;
...
let sql_getIngredientIdUponInsertion =
  sql_insertSubIngredientIds + sql_selectLastInsertId;
...
let sql_insertInventoryColumn = `alter table ?? add column ?? int not null default 0;`;

let sql_updateInventory = `update ?? set ??=? where id=?;`;

let sql_getServings = `select servings from ${dbSetting.table_member} 
    where id=1;`;
